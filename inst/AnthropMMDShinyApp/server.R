shinyServer(function(input, output, session) {
	...
	observeEvent(input$loadData, {
		if (! is.null(input$file$name)) { # on verifie que l'utilisateur a bien choisi un fichier !
			...
			if (validDataMMD(dat, type=input$typeData)) { # on regarde si le fichier est bien valide
				...
				dat <- tableToFreq(dat)
				updateSliderInput(session, "minNbInd", value=min(10, max3(dat)), min=1, max=max3(dat)) # on empeche par la suite la saisie de valeurs "nb min individus" trop elevees au vu des donnees
	...
	output$download_tableIMD <- downloadHandler(filename='Individual_MD_matrix.csv', content=function(file) {
	...
	output$plotMDS <- renderPlot({
					if (input$loadData>0 & exists("dat", envir=myenvg) & length(input$selectGroups)>2) { # si un jeu de donnees valide a bien ete fourni et qu'on a au moins 3 groupes !
						...
						} else if (ncol(coor)==2 & any(mmdval<0)) { 
							plot(x=0, y=0, xlab="", ylab="", axes=FALSE, xlim=c(-2,2), ylim=c(-2,2), pch="")
							text(x=0, y=0.5, labels="There are negative values in the MMD matrix.", col="black")
							text(x=0, y=-0.5, labels="Remove variables with a negative contribution to get a MDS plot.", col="black")
